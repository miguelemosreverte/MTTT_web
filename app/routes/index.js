...
router.use(expressSession({secret:'somesecrettokenhere', resave: false, saveUninitialized: true}));
...
router.post('/CorpusPreparation', function(req, res) {
    ...
    request.post('http://0.0.0.0:5000/PrepareCorpus').form({
...
router.post('/Evaluation', function(req, res) {
  request.post('http://0.0.0.0:5000/Evaluate').form({
      WER : req.body.WER,
      PER : req.body.PER,
      HTER : req.body.HTER,
      BLEU : req.body.BLEU,
...
router.get('/GetLM', function(req, res,next) {
      req.pipe(request.get('http://0.0.0.0:5000/GetLM/'+ req.query.LM_name)).pipe(res)
});

router.post('/Translate', function(req, res) {
    request.post('http://0.0.0.0:5000/Translate').form({
        LM_name : req.body['LM_name'],
...
router.get('/GetAvailableLMs', function(req, res) {
      request.get('http://0.0.0.0:5000/GetAllAvailableLanguageModelNames').pipe(res)
});


router.get('/GetAvailableLanguages', function(req, res) {
      request.get('http://0.0.0.0:5000/GetAvailableLanguages').pipe(res)
});

router.post('/Train', function(req, res) {
    request.get('http://0.0.0.0:5000/Train/' + req.session.LM_name + '/' + req.session.source_lang +'/' + req.session.target_lang
    ).pipe(res)
});
